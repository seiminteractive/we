<template>
  <div id="app-root">
    ...
    <div class="page-stack" :class="{ 'is-news-hidden': !noticiasVisible }">
      <QuienesSomosSection />
      <ParaQuienSection />
      <ServiciosSection />
      <QueHacemosSection />
...
import { useSiteSettings } from '../lib/firebase'
import { marcarVisitada } from '../lib/intro'
...
onMounted(() => {
  ...
  // onMounted del padre corre despues del de los hijos, asi que IntroOverlay y
  // Hero ya leyeron la bandera en true. A partir de aca, las visitas siguientes
  // de esta carga de pagina entran sin animacion de entrada.
  marcarVisitada()
})

// Al apagar el interruptor desde el panel, #noticias se desmonta y su tween
// queda apuntando a un elemento que ya no existe. Se reconstruyen todos y
// se remide, porque ademas cambian las alturas de las secciones siguientes.
watch(noticiasVisible, () => {
  nextTick(() => {
    buildBends()
    setTimeout(() => ScrollTrigger.refresh(), 300)
  })
...
/* Sin Novedades, #contacto entra despues del bloque grafito "Como trabajamos",
   asi que su cap curvo tiene que usar ese color para no cortar la transicion. */
.page-stack.is-news-hidden #contacto::before {
  background: #373737;
}
</style>
